...
```
 ...
 From the loading values in the table, we can say that PC1 represents an average of all self-esteem scores and PC2 represents the difference between qualities which can be termed as self-respect (Esteem questions 8, 9 and 10) and those which can be categorised as associated with self-perception(1-7).
  ...
  We can summarize the PC1 score of a Subject $n$ with the following formula:
$$PC1_n = `r paste0( format( pca_loadings[ , 1 ], nsmall = 3 ), '*', paste0( 'E', 1:10, '_n' ), collapse="+")`.$$
...
```{r run regressions, results='asis'}
...
# Case study 3: Auto data set (leave for next week)
...
```{r, echo=F}
ggplot() +
  geom_abline(mapping=aes(intercept = 1, slope = 1.2,
                          linetype='True Line')) +
  geom_abline(mapping=aes(intercept = coef(modelQ412)[1], slope = coef(modelQ412)[2],
                          linetype='Estimated Line')) +
  geom_point(mapping=aes(x=x,y=y)) +
  labs(title = TeX('Plotting $x$ Values Against $y$ Values Generated from the Model $y = 1 + 1.2x + \\epsilon$'),
       linetype='') +
  theme_minimal()
...
i. Provide a residual plot where fitted $\mathbf{y}$-values are on the x-axis and residuals are on the y-axis. 

Here is the residuals plot

```{r, echo=F}
ggplot() +
  geom_point(mapping=aes(x=fitted(modelQ412),y=resid(modelQ412))) +
  labs(title = TeX('Plotting Fitted Values ($\\hat{y}$) Against residuals ($e$)'),
       x='Fitted Values',
       y='Residuals') +
  theme_minimal()
```
...
```{r, echo=F}
ggplot() +
  geom_density(data=filter(cleanedCoefficients,term=='x'),
               mapping=aes(x=estimate-mean(estimate),linetype='Sampling Distribution')) +
  geom_line(mapping=aes(x=seq(-4,4,0.1),y=dt(seq(-4,4,0.1),38),linetype='Theoretical Distribution')) +
  labs(title="Comparing the Sampling and the Theoretical Distribution for the Model's Slope",
  x=TeX('$\\hat{\\beta} - \\bar{\\beta}$'),
       y='Density',
       linetype='') +
  theme_minimal()
```
...
cleanedCoefficients %>%
  ...
  geom_segment(mapping=aes(y=simNum,yend=simNum,x=lowerBound,xend=upperBound)) +
  labs(title="95% Confidence Intervals for the Model's Slope",
       y='',
       x='Slope') +
  theme_minimal() +
...
We find that `r propContains` of the intervals contain the true value, very close to the expected proportion (0.95).
